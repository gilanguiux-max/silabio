...
class M_buat_peminjaman extends CI_Model {
    ...
    {
        $id_nama_bahan = $data['nama_bahan'];
        $id_nama_alat = $data['nama_alat'];
        $sql = "UPDATE kelola_bahan, kelola_alat
                SET kelola_bahan.stock = kelola_bahan.stock + 1,
                kelola_alat.stok = kelola_alat.stok + 1
                WHERE kelola_bahan.id_nama_bahan = $id_nama_bahan
                AND kelola_alat.id_nama_alat = $id_nama_alat ";
        $this->db->query($sql);
    }
